...
let matmul = (m, v) => [m[0][0] * v.x + m[0][1] * v.y, m[1][0] * v.x + m[1][1] * v.y];
let inv = (m) => {
  let [[a, b], [c, d]] = m;
  let det = 1 / (a * d - b * c);
  return [
...
export function vg(ctx) {
  ...
  ctx.vgT = (vx, vy) => {
    ...
    let gx = parseFloat(ilx(interp(lx(Axis.X.p1.value), lx(Axis.X.p2.value), m)).toPrecision(8));
    let gy = parseFloat(ily(interp(ly(Axis.Y.p1.value), ly(Axis.Y.p2.value), n)).toPrecision(8));
    return [gx, gy];
  };
  ctx.gvT = (gx, gy) => {
...
let conv = (img, k3x3) => {
  let newImg = [];
  for (let j = 0; j < img.height - 2; j++) newImg.push(new Array(img.width - 2).fill(0));
